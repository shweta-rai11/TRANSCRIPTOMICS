#!/usr/bin/env python3
"""
organize_figures_by_section.py — copies each existing figure into
results/figures/by_section/, renamed to <section-slug><n>.png, and writes
results/figures/by_section/INDEX.md mapping new name -> original file ->
producing script -> thesis section.

Every (original_file, script) pair below was confirmed by grep against the
actual ggsave()/png()/pdf()/image_write() call in that script — none is
guessed from the filename. Sections with no entries here have no figure on
disk because the generating script has never been run (confirmed by
`find results/figures -iname ...` turning up nothing) — they are listed as
MISSING at the bottom, not silently omitted.
"""
import os, shutil
...
# Sections confirmed (by grep + disk search) to have NO figure at all, because
# the producing script has never been executed, or (for §3.7) produces no
# figures by design.
MISSING = [
